...
Another challenge was that we designed the bounding box trigger method with the intention of implementing line following first, then using the line following method to guide the Neato along each path as it explores the entire maze. However, this method did not scale up to the full maze because following a maze requires more precise direction and trigger methods than line following does. Thus, we made a mistake by starting the project with line following and assuming that would be applicable throughout the entire process. To optimize the maze solver, we should have seperated the maze 'triggers', which detect the presence of paths, from the maze 'sensors', which tell the Neato when to turn.
